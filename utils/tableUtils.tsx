    ...
    for (const answer of response.answers) {
      if (answer.question.type === QuestionType.Select)
        ...
        switch (answer.question.type) {
          case QuestionType.Date: data[answer.question.id] = answer.answerDate ? new Date(answer.answerDate) : undefined; break;
          ...
          case QuestionType.Scale: data[answer.question.id] = answer.answerSelect[0]?.value ?? undefined; break;
        }
      }
    }
    result.push(data);
...
const dateFormatter: Tabulator.Formatter = (cell: Tabulator.CellComponent, formatterParams, onRender) => {
  const date = cell.getValue() as Date;
  return date?.toLocaleDateString() ?? "";
}

const dateSorter = (a: Date, b: Date, _aRow, _bRow, _col, dir: "asc" | "desc") => {
  ...
  return survey.questions.map<ColumnDefinition>(question => {
    switch (question.type) {
      ...
      case QuestionType.Int: {
        return { title: question.questionText, field: question.id, headerSortTristate: true };
      }
      case QuestionType.Num: {
        return { title: question.questionText, field: question.id, headerSortTristate: true };
      }
      case QuestionType.Bool: {
        return { title: question.questionText, field: question.id, formatter: "tickCross", formatterParams: { allowEmpty: true }, headerSortTristate: true };
      }
      ...
      case QuestionType.Select: {
        return {
          title: question.questionText,
          columns: question.selectOptions.map<ColumnDefinition>(selectOption => (
            {
